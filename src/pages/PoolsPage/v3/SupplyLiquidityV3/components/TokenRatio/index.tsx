...
export function TokenRatio({
  ...
  return (
    <div
      className={'preset-ranges-wrapper pl-1 mxs_pl-0 mxs_mb-1 ms_pl-0 ms_mb-1'}
    >
      <div className='mb-1 f f-ac'>
        <Divide style={{ display: 'block', fill: 'currentcolor' }} size={15} />
        <span className='ml-05'>Token ratio</span>
      </div>
      <div className='f full-h pos-r'>
        <div
          className='token-ratio f ms_w-100'
          style={{ opacity: isEmpty ? '0.5' : '1' }}
        >
          <div
            className='token-ratio__part full-h'
            style={{
              width: `${token0Ratio}%`,
              background: '#707eff',
              borderRadius: +token0Ratio === 100 ? '8px' : '8px 0 0 8px',
            }}
          ></div>
          <div
            className='token-ratio__part full-h'
            style={{
              width: `${token1Ratio}%`,
              background: '#ec92ff',
              borderRadius: +token1Ratio === 100 ? '8px' : '0 8px 8px 0',
            }}
          ></div>
        </div>
      </div>
      <div className='mt-1' style={{ opacity: isEmpty ? 0.5 : '1' }}>
        <div className='f mb-1 f-ac'>
          ...
          <div className='ml-a mr-1'>{`${Number(token0Ratio).toPrecision(
            3,
          )}%`}</div>
          <div className=''>
            <button
            ...
            <button
              ...
              disabled={disableLeftIncButton || incrementDisabled || disabled}
              className='range-input__btn'
            >
              +
            </button>
        ...
        <div className='f f-ac'>
          <div
            style={{
              ...
              height: '16px',
              borderRadius: '50%',
              backgroundColor: '#ec92ff',
            }}
          ></div>
          <div className='ml-05'>{currencyB?.symbol}</div>
          <div className='ml-a mr-1'>{`${
            isEmpty ? 0 : (100 - +token0Ratio).toPrecision(3)
          }%`}</div>
          <div className=''>
            <button
              onClick={handleDecrementRight}
              disabled={disableRightDecButton || decrementDisabled || disabled}
              className='range-input__btn'
            >
              +
            </button>
