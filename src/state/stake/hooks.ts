...
const getBulkPairData = async (pairList: any) => {
  ...
  try {
    ...
    let oneDayData = oneDayResult?.data?.pairs.reduce(
...
const getOneDayVolume = async () => {
  ...
  let healthInfo = await healthClient.query({
    query: SUBGRAPH_HEALTH,
  });
  let current = Number(
    healthInfo.data.indexingStatusForCurrentVersion.chains[0].latestBlock.number
  );
