...
import { ADDRESS_ZERO } from './utils/v3constants';
...
export abstract class UniV3NonfungiblePositionManager extends SelfPermit {
  ...
  public static addCallParameters(
    ...
    if (options.token0Permit) {
      calldatas.push(
        UniV3NonfungiblePositionManager.encodePermit(
    ...
    if (isMint(options)) {
      ...
      calldatas.push(
        UniV3NonfungiblePositionManager.INTERFACE.encodeFunctionData('mint', [
          {
  ...
  public static collectCallParameters(
    ...
    const calldatas: string[] = UniV3NonfungiblePositionManager.encodeCollect(
      options,
    );
  ...
  public static removeCallParameters(
    ...
    if (options.permit) {
      calldatas.push(
        UniV3NonfungiblePositionManager.INTERFACE.encodeFunctionData('permit', [
    ...
    calldatas.push(
      ...UniV3NonfungiblePositionManager.encodeCollect({
    ...
    if (options.liquidityPercentage.equalTo(ONE)) {
      if (options.burnToken) {
        calldatas.push(
          UniV3NonfungiblePositionManager.INTERFACE.encodeFunctionData('burn', [
            tokenId,
          ]),
        );
      }
    ...
    return {
      calldata: UniV3NonfungiblePositionManager.INTERFACE.encodeFunctionData(
        'multicall',
        [calldatas],
      ),
      value: toHex(0),
  ...
  private static encodeCollect(options: CollectOptions): string[] {
    ...
    // collect
    calldatas.push(
      UniV3NonfungiblePositionManager.INTERFACE.encodeFunctionData('collect', [
        {
          tokenId,
