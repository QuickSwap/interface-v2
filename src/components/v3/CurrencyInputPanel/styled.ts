...
export const InputRow = styled.div<{
  ...
  align-items: center;
  padding: ${({ hideCurrency }) =>
    hideCurrency ? '1rem 1rem 0.75rem 0' : '1rem 1rem 0.75rem 1rem'};
  ${({ theme }) => theme.mediaWidth.upToExtraSmall`{
...
export const FiatRow = styled(LabelRow)<{
  ...
  padding: ${({ shallow, page }) =>
    page === 'addLiq' ? '0 1rem' : shallow ? '0' : '0 1rem 1rem'};
...
export const MaxButton = styled.button<{ page: string | undefined }>`
  background-color: ${({ page, theme }) =>
    page === 'addLiq' ? 'var(--primary)' : 'var(--primary-weak)'};
  border-radius: 6px;
  color: white;
  margin-right: 10px;
  border: none;
