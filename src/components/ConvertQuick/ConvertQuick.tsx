...
import styles from 'styles/components/ConvertQuick.module.scss';
import { ChainId } from '@uniswap/sdk';
import { OLD_QUICK } from 'constants/v3/addresses';

interface ConvertQuickProps {
  isWidget?: boolean;
}

const ConvertQuick: React.FC<ConvertQuickProps> = ({ isWidget }) => {
  ...
  return (
    <Box
      ...
      id='convertQUICKPage'
    >
      <h1 className={`${isWidget ? styles.widgetHeader : 'h4'}`}>
        {t('convert')} QUICK
      </h1>
      <Box
        className={`${styles.convertQUICKWrapper} ${
          isWidget === true ? '' : styles.convertPage
        }`}
      >
