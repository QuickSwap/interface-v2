...
const LeaderboardWrapper: React.FC<Props> = (props: Props) => {
  ...
  const pastWinnersEnabled = config['pastwinners']['available'];
  const history = useHistory();

  useEffect(() => {
    if (showPastWinners ? !pastWinnersEnabled : !showLeaderBoard) {
      history.push('/');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showLeaderBoard, showPastWinners]);
    ...
    <Box width='100%' mb={3} id='contest-page'>
      <Box className='pageHeading'>
        <Box className='flex items-center row'>
          <h4>{showPastWinners ? t('pastWinner') : t('leaderBoard')}</h4>
        </Box>
