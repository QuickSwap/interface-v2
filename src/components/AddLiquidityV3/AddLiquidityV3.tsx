...
export default function AddLiquidity(
  ...
  // }: RouteComponentProps<{ currencyIdA?: string; currencyIdB?: string; feeAmount?: string; tokenId?: string }>
    ...
    const allowedSlippagePercent: Percent = 
    useMemo(() => { return new Percent(JSBI.BigInt(allowedSlippagePercent), JSBI.BigInt(10000))}, [allowedSlippage]);
                ...
                {mustCreateSeparately && (
                    <ButtonError disabled={!isValid || attemptingTxn || !position}>
                        {attemptingTxn ? (
                            ...
                            <Text fontWeight={500}>{errorMessage ? errorMessage : 'Create'}</Text>
                        )}
                    </ButtonError>
                )}
                <button
                ...
                <div className={"add-liquidity pt-1 mt-1"} data-wide={!hasExistingPosition && !networkFailed}>
                    <AutoColumn gap="lg">
                        {hasExistingPosition && existingPosition && (
                            <PositionPreview position={existingPosition} title={'Selected Range'} inRange={!outOfRange} ticksAtLimit={ticksAtLimit} />
                        )}
                    </AutoColumn>
