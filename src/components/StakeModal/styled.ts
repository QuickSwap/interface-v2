...
export const NFTPositionsRow = styled.div`
  ...
  margin-bottom: 8px;
`;
export const NFTPosition = styled.div<{
  selected?: boolean;
  skeleton?: boolean;
}>`
  display: inline-flex;
  ...
  border-radius: 1rem;
  border: 1px solid
    ${({ selected }) => (selected ? '#3970FF' : 'rgba(60, 97, 126, 0.5)')};
  padding: 8px;
  margin-right: 9px;
  ...
  `}
`;
export const NFTPositionSelectCircle = styled.div<{
  selected?: boolean;
  skeleton?: boolean;
}>`
  position: absolute;
...
`;
export const NFTPositionIcon = styled.div<{
  skeleton?: boolean;
  name?: string;
}>`
  width: 40px;
