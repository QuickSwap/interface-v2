...
export default function OrdersList(props) {
  ...
  const renderEmptyRow = useCallback(() => {
    ...
    return (
      <tr>
        <td colSpan="5"><div className="Exchange-empty-positions-list-note">No open orders</div></td>
      </tr>
    );
  }, [orders]);
          ...
          <tr className="Exchange-list-item" key={`${order.type}-${order.index}`}>
            ...
            <td>
              ...
              {formatAmount(
                ...
                fromTokenInfo.isStable || fromTokenInfo.isUsdq ? 2 : 4,
                true
              )}{" "}
              ...
              {formatAmount(
                ...
                toTokenInfo.isStable || toTokenInfo.isUsdq ? 2 : 4,
                true
              )}{" "}
              {toTokenInfo.symbol}
            </td>
      ...
      return (
        <tr className="Exchange-list-item" key={`${order.isLong}-${order.type}-${order.index}`}>
          ...
          <td>
            ...
            &nbsp;by ${formatAmount(order.sizeDelta, USD_DECIMALS, 2, true)} {receive}
