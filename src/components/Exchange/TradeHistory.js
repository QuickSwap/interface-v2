...
export default function TradeHistory(props) {
  let { account, infoTokens,getTokenInfo, chainId, nativeTokenAddress } = props;
  Object.keys(infoTokens).forEach(k=> infoTokens[k.toLowerCase()] = infoTokens[k]);
  
  const{ trades } = useTradesFromGraph(chainId, account);
  ...
  const getMsg = useCallback(
    (trade) => {
      ...
      let paramRaw=tradeData.params;
      if(tradeData.action==="CreateSwapOrder" && tradeData.params.indexOf("path")>0)
        paramRaw = tradeData.params.substring(0,tradeData.params.indexOf("path")-1)+tradeData.params.substring(tradeData.params.indexOf("orderIndex")-1,tradeData.params.length)
      const params = JSON.parse(paramRaw);
      if(params.isLong)  
        params.isLong = params.isLong === "true";
      if(params.triggerAboveThreshold) 
        params.triggerAboveThreshold = params.triggerAboveThreshold === "true";
        
      const defaultMsg = "";

      if (tradeData.action === "BuyUSDQ") {
      ...
      if (tradeData.action === "SellUSDQ") {
        const token = getTokenInfo(infoTokens, params.token, true, nativeTokenAddress);
        if (!token) {
          return defaultMsg;
        }
        return `Swap ${formatAmount(params.usdqAmount, 18, 4, true)} USDQ for ${formatAmount(
          params.tokenAmount,
          token.decimals,
          4,
          true
              ...
              <div>
                <div className="muted TradeHistory-time">
                  ...
                  {(!account || account.length === 0) && (
                    <span>
                      {" "}
                      (<Link to={`/actions/${tradeData.account}`} className="textcolor">{tradeData.account}</Link>)
                    </span>
                  )}
                </div>
