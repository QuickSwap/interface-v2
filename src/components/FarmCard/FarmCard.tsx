...
const FarmCard: React.FC<{ stakingInfo: StakingInfo }> = ({ stakingInfo }) => {
  ...
  const valueOfUnstakedAmountInUSDC =
    valueOfUnstakedAmountInBaseToken && USDPrice?.quote(valueOfUnstakedAmountInBaseToken)
  ...
  const { parsedAmount, error } = useDerivedStakeInfo(stakeAmount, stakingInfo.stakedAmount.token, userLiquidityUnstaked)
        ...
        <Box width='100%' mt={2.5} pl={4} pr={4} pt={4} display='flex' flexDirection='row' borderTop='1px solid #444444' alignItems='center' justifyContent='space-between'>
          <Box width={0.25} ml={4} mr={4} style={{color: '#696c80'}}>
            <Box display='flex' flexDirection='row' alignItems='flex-start' justifyContent='space-between'>
              ...
              <Box display='flex' flexDirection='column' alignItems='flex-end' justifyContent='flex-start'>
                <Typography variant='body2'>{ userLiquidityUnstaked ? userLiquidityUnstaked.toSignificant(2) : 0 } LP <span>(${valueOfUnstakedAmountInUSDC?.toSignificant(2)})</span></Typography>
                <Link to='#' style={{color: '#448aff'}}>Get {currency0.symbol} / {currency1.symbol} LP</Link>
              </Box>
            </Box>
            <Box className={classes.inputVal} mb={2} mt={2} p={2}>
              <input placeholder='0.00' value={stakeAmount} onChange={(evt: any) => { setStakeAmount(evt.target.value) }} />
              <Typography
                variant='body2'
                style={{ color: userLiquidityUnstaked && userLiquidityUnstaked.greaterThan('0') ? '#448aff' : '#636780'}}
                onClick={() => {
          ...
          <Box width={0.25} ml={4} mr={4} style={{color: '#696c80'}}>
            <Box display='flex' flexDirection='row' alignItems='flex-start' justifyContent='space-between'>
              ...
              <Typography variant='body2'>{ stakingInfo.stakedAmount.toSignificant(2) } LP <span>(${valueOfMyStakedAmountInUSDC?.toSignificant(2)})</span></Typography>
            </Box>
            <Box className={classes.inputVal} mb={2} mt={4.5} p={2}>
              <input placeholder='0.00' value={unstakeAmount} onChange={(evt: any) => { setUnStakeAmount(evt.target.value) }} />
              <Typography
                variant='body2'
                style={{ color: stakingInfo.stakedAmount && stakingInfo.stakedAmount.greaterThan('0') ? '#448aff' : '#636780'}}
                onClick={() => {
          ...
          <Box width={0.25} ml={4} mr={4} style={{color: '#696c80'}}>
            ...
            <Box
              className={stakingInfo.earnedAmount.greaterThan('0') ? classes.buttonClaim : classes.buttonToken}
              mb={2}
              p={2}
              onClick={() => {
