...
const SwapProInfo: React.FC<{
  ...
  const fetchTokenData = async (tokenAddress?: string) => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_LEADERBOARD_APP_URL}/analytics/top-token-details/${tokenAddress}/v2?chainId=${chainIdToUse}`,
    );
    let tokenV2;
    if (res.ok) {
    ...
    const v3Res = await fetch(
      `${process.env.NEXT_PUBLIC_LEADERBOARD_APP_URL}/analytics/top-token-details/${tokenAddress}/v3?chainId=${chainIdToUse}`,
    );
    if (!v3Res.ok) {
      return null;
    }
