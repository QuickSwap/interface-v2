...
export default function PositionCardBodyStat({
  rewardToken,
  earned,
  bonusRewardToken,
  bonusEarned,
}: PositionCardBodyStatProps) {
  const rewardList = useMemo(() => {
    if (rewardToken.id === bonusRewardToken.id)
      return [{ token: rewardToken, amount: +earned + +bonusEarned }];
      ...
      <div className='f ms_fd-c mxs_fd-c'>
        {rewardList.map((reward: any, i) => (
          <div
            key={i}
            className={
              'f f-ac mr-1 mxs_mr-0 mxs_mt-1 mxs_mb-0 ms_mr-0 ms_mt-1 ms_mb-0 position-card-body-stat'
            }
          >
            <CurrencyLogo
              size={'30px'}
            ...
            >{`${formatReward(reward.amount)} ${reward.token.symbol}`}</div>
          </div>
        ))}
      </div>
    </div>
