...
export const TokenIcon = styled.div<{
  ...
  logo: string;
    ...
    background-color: ${({ name }) =>
      name ? stringToColour(name).background : '#5aa7df'};
    border: 1px solid ${({ name }) =>
      name ? stringToColour(name).border : '#5aa7df'};
...
export const MoreButton = styled.button<{ single?: boolean }>`
  ...
  &:last-of-type {
    margin: ${({ single }) =>
      single ? 'auto 0 auto auto' : 'auto 0 auto 1rem'};
  }
...
export const EventProgressInner = styled.div.attrs(
