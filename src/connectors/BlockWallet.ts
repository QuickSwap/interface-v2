...
type BlockWalletProvider = Provider & {
  isBlockWallet?: boolean;
  isConnected?: () => boolean;
  detected?: BlockWalletProvider[];
  providers?: BlockWalletProvider[];
  chainId?: string;
};

export class NoBlockWalletError extends Error {
  public constructor() {
    super('BlockWallet not installed');
    this.name = NoBlockWalletError.name;
    Object.setPrototypeOf(this, NoBlockWalletError.prototype);
  }
}
 ...
 */
export interface BlockWalletConstructorArgs {
  actions: Actions;
  options?: Parameters<typeof detectEthereumProvider>[0];
  onError?: (error: Error) => void;
}

export class BlockWallet extends Connector {
      ...
      async (_) => {
        ...
        if (provider) {
          ...
          if (this.provider.detected?.length) {
            this.provider =
              this.provider.detected.find((p) => p.isBlockWallet) ??
              this.provider.detected[0];
          } else if (this.provider.providers?.length) {
            this.provider =
              this.provider.providers.find((p) => p.isBlockWallet) ??
  ...
  public async activate(
    ...
    return this.isomorphicInitialize()
      .then(async () => {
        if (!this.provider) throw new NoBlockWalletError();
