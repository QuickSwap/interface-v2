    ...
    async function fetchGlobalData() {
      if (chainId && v2) {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_LEADERBOARD_APP_URL}/analytics/global-data/v2?chainId=${chainId}`,
        );
        if (!res.ok) {
      ...
      if (v3 && chainId) {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_LEADERBOARD_APP_URL}/analytics/global-data/v3?chainId=${chainId}`,
        );
        if (!res.ok) {
          const errorText = await res.text();
          throw new Error(
